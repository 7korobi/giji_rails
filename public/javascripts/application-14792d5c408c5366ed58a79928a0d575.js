...
AJAX = function($scope) {
  ...
  return $scope.ajax_event = function(turn, href, is_news) {
...
var Diary, DiaryHistory, FixedBox, Form;

angular.module("giji.config", [], function($routeProvider, $locationProvider) {
...
FixedBox = (function() {
  ...
  FixedBox.prototype.scroll = function() {
    ...
    this.box.to_z_front();
    if (1 < win.zoom || head.browser.android || this.force_zoom) {
      this.box.css({
        position: "absolute"
      });
...
angular.module("giji.directives").directive("theme", [
  "$compile", function($compile) {
    return {
      ...
      link: function($scope, elm, attr, ctrl) {
        ...
        return GIJI.template($compile, $scope, elm, "navi/css");
      }
    };
  }
]);
...
var INIT, INIT_FACE, INIT_MESSAGES, INIT_POTOFS, INIT_STORY;
  ...
  if (new_base.turn != null) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if (message.plain != null) {
        ...
        if (message.plain.updated_at != null) {
      ...
      _results.push(message.updated_at || (message.updated_at = new Date));
    }
    return _results;
  }
};
...
INIT_POTOFS = function($scope, gon) {
  var bonds, is_dead_lose, is_lone_lose, mask, potof, pseudobonds, rolestate, text, win_check, winner, _i, _j, _len, _len1, _ref, _ref1, _ref2, _ref3, _ref4, _results;
  if (gon.potofs != null) {
    _ref = gon.potofs;
    _results = [];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if (potof.role != null) {
        ...
        if (gon.story != null) {
          win_check = function(potof, story) {
            var win, win_by_role, win_juror, win_love, win_zombie, zombie, _ref1;
            win_by_role = function(list) {
              var role, win, _j, _len1, _ref1, _ref2;
              _ref1 = potof.role;
              for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
                role = _ref1[_j];
                win = (_ref2 = list[role]) != null ? _ref2.win : void 0;
                if (win) {
                  return win;
                }
              }
            ...
            win_love = (_ref1 = SOW.loves[potof.love]) != null ? _ref1.win : void 0;
          ...
          if (gon.story.is_finish) {
            winner = ((_ref1 = gon.event) != null ? _ref1.winner : void 0) || (gon.events != null) && ((_ref2 = _.last(gon.events)) != null ? _ref2.winner : void 0);
        ...
        potof.win_name = (_ref3 = SOW.wins[potof.win]) != null ? _ref3.name : void 0;
      ...
      potof.summary = function() {
        switch ($scope.potofs_order.value) {
          ...
          case 'win_name':
          case 'role_names':
            return potof.role_names.join('、');
          case 'select_name':
            if (potof.select_name) {
              return potof.select_name + "を希望";
            } else {
              return "";
            }
            break;
          case 'text':
            return potof.text.join('');
        }
      };
      _results.push(potof);
    }
...
angular.module("giji.directives").directive("diary", [
  "$compile", function($compile) {
    return {
      restrict: "A",
      link: function($scope, elm, attr, ctrl) {
  ...
  function() {
    return {
      ...
      link: function($scope, elm, attr, ctrl) {
        ...
        return elm.on('click', 'dt', function() {
...
angular.module("giji.directives").directive("navi", [
  "$compile", function($compile) {
    var effect, extra_navis, static_navis;
    effect = function($scope) {
      var resize_naviitems;
      resize_naviitems = function() {
        var calculate, content, height, info_left, info_right, k, lim_left, outframe, v, width, _ref;
        if (!(($scope.navi != null) && ($scope.width != null))) {
          return;
        }
        width = win.width;
        height = win.height;
        ...
        calculate = function(key, params) {
          var buttons, element, gap, info_width, max_width, small;
        ...
        return $("#outframe")[0].className = outframe;
      };
    ...
    return {
      restrict: "A",
      link: function($scope, elm, attr, ctrl) {
        var attr_id, child, _i, _len, _ref, _results;
        ...
        $scope.navi.popstate();
        if (attr.child != null) {
          _ref = attr.child.split(",");
          _results = [];
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            child = _ref[_i];
            _results.push(extra_navis[child] = {
              parent: attr.navi
            });
          }
          return _results;
...
angular.module("giji.directives").directive("log", [
  "$compile", function($compile) {
    return {
      restrict: "A",
      link: function($scope, elm, attr, ctrl) {
        ...
        log.cancel_btn = function() {
          if ((log.logid != null) && "q" === log.logid[0]) {
            if ((new Date() - log.updated_at) < 25 * 1000) {
              return "<a class=\"mark\" href_eval='cancel_say(\"" + log.logid + "\")'>なら削除できます。⏳</a>";
            } else {
              return "";
            }
          } else {
...
POTOFS = function($scope) {
  ...
  potofs_sortBy = function(tgt, reverse) {
    var groups, has_head, items, key, keys, list, order, orders, _i, _len;
    if (!$scope.potofs) {
      return;
    }
    list = _.sortBy($scope.potofs, function(o) {
      return o[tgt];
    });
    if (reverse) {
      list.reverse();
    ...
    $scope.potofs = list;
    groups = $scope.potofs_groups = _.groupBy($scope.potofs, function(o) {
      return o[tgt];
    });
    keys = _.uniq(_.map($scope.potofs, function(o) {
      return o[tgt];
    }));
...
angular.module("giji.directives").directive("template", [
  "$interpolate", "$compile", function($interpolate, $compile) {
    var html, idx, key, val, _i, _len, _ref;
...
angular.module("giji.directives").directive("listup", [
  "$compile", function($compile) {
    return {
      restrict: "A",
      link: function($scope, elm, attr, ctrl) {
...
angular.module("giji.directives").directive("form", [
  "$compile", function($compile) {
    return {
      restrict: "A",
      link: function($scope, elm, attr, ctrl) {
...
var TOKEN_INPUT, tokenInput;

tokenInput = {};

TOKEN_INPUT = function($scope) {
  var doIt;
  $scope.tokenInput = function(target, all, obj) {
  ...
  doIt = function() {
    ...
    if (target.length > 0 && ($scope.story.card.event != null)) {
      return $scope.tokenInput('#eventcard', SOW.events, $scope.story.card.event);
    }
  };
...
MODULE = function($scope, $filter) {
  ...
  $scope.text_decolate = function(log) {
    if (log) {
      return background(anchor(link(random(log))));
    } else {
      return null;
    }
  };
...
jQuery(function() {
  ...
  FixedBox.push(-1, 1, '#topviewer', head.browser.iphone);
  FixedBox.push(1, -1, '#sayfilter');
  return FixedBox.push(-8, -8, '#buttons');
});
